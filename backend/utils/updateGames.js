    ...
    export const updateGames = async (date) => {
      ...
      try {
        ...
        for (const event of events) {
          ...
          // Check if preview exists in the database
          // const checkPreviewQuery = 'SELECT preview FROM events WHERE event_id = $1';
          // const checkPreviewResult = await pool.query(checkPreviewQuery, [event_id]);
          // let preview = checkPreviewResult.rows[0]?.preview;
          let preview = '';
    
          // Fetch game preview using gameReviewScraper only if preview is empty or null
          ...
          const query = `
            ...
            ON CONFLICT (event_id) DO UPDATE SET
              ...
              preview = COALESCE(EXCLUDED.preview, events.preview),
